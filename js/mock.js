document.addEventListener("DOMContentLoaded", () => {
  // --- ESTADO DA APLICAÇÃO ---
  // As chaves de API são mantidas, mas não serão usadas na busca de clima/poluição
  ...
  // --- DADOS MOCKADOS PARA TESTE ---
   const mockApiData = {
    "cidade teste 1": { // RISCO MUITO ALTO
        weather: { main: { temp: 8, humidity: 20, feels_like: 6 }, wind: { speed: 8.3 }, name: "Fornalha Seca", weather: [{ description: "Névoa", icon: "50d" }] },
        pollution: { list: [{ main: { aqi: 5 }, components: { pm2_5: 65.0, no2: 150, o3: 200 } }] },
        imageUrl: 'https://images.unsplash.com/photo-1534274988757-a28bf1a57c17?q=80&w=1935&auto=format&fit=crop' // Imagem de cidade com névoa

    },
    "cidade teste 2": { // RISCO ALTO
        weather: { main: { temp: 10, humidity: 25, feels_like: 9 }, wind: { speed: 3.3 }, name: "Pico Congelante", weather: [{ description: "Nublado", icon: "04d" }] },
        pollution: { list: [{ main: { aqi: 4 }, components: { pm2_5: 50.1, no2: 120, o3: 150 } }] },
        imageUrl: 'https://images.unsplash.com/photo-1485470733090-0aae1788d5af?q=80&w=2117&auto=format&fit=crop'
    },
    "cidade teste 3": { // RISCO MODERADO
        weather: { main: { temp: 20, humidity: 38, feels_like: 20 }, wind: { speed: 4.1 }, name: "Vale Intermediário", weather: [{ description: "Poucas Nuvens", icon: "02d" }] },
        pollution: { list: [{ main: { aqi: 3 }, components: { pm2_5: 38.2, no2: 50, o3: 90 } }] },
        imageUrl: 'https://images.unsplash.com/photo-1501854140801-50d01698950b?q=80&w=1950&auto=format&fit=crop' // Imagem de vale com nuvens
    },
    "cidade teste 4": { // RISCO BAIXO
        weather: { main: { temp: 24, humidity: 60, feels_like: 24 }, wind: { speed: 2.7 }, name: "Brisa Pura", weather: [{ description: "Céu Limpo", icon: "01d" }] },
        pollution: { list: [{ main: { aqi: 1 }, components: { pm2_5: 10.5, no2: 15, o3: 40 } }] },
        imageUrl: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?q=80&w=2070&auto=format&fit=crop'
    },
    "cidade teste 5": { // OUTRO RISCO BAIXO
        weather: { main: { temp: 28, humidity: 75, feels_like: 30 }, wind: { speed: 1.5 }, name: "Praia Tranquila", weather: [{ description: "Ensolarado", icon: "01d" }] },
        pollution: { list: [{ main: { aqi: 1 }, components: { pm2_5: 5.8, no2: 10, o3: 30 } }] },
        imageUrl: 'https://images.unsplash.com/photo-1473496169904-658ba7c44d8a?q=80&w=2070&auto=format&fit=crop'
    },
     "cidade teste 6": { // RISCO MÁXIMO (SCORE 80)
        weather: { main: { temp: 5, humidity: 15, feels_like: 2 }, wind: { speed: 10 }, name: "Cratera Vulcânica", weather: [{ description: "Cinzas Vulcânicas", icon: "50d" }] },
        pollution: { list: [{ main: { aqi: 5 }, components: { pm2_5: 150.0, no2: 200, o3: 250 } }] },
        imageUrl: 'https://images.unsplash.com/photo-1604782352199-22776c53547b?q=80&w=1935&auto=format&fit=crop' // Imagem de vulcão
    }
  };


  // --- SELETORES GERAIS E DE ELEMENTOS (código original) ---
  ...
  // --- TODAS AS FUNÇÕES ORIGINAIS (tema, formulários, estação do ano, etc.) ---
  // (O código abaixo é o seu original, para que nada mais quebre)
  ...
  const getSeasonInfo = () => {
    ...
    const inicioVerao = new Date(year, 11, 21);
    const inicioOutono = new Date(year, 2, 20);
    const inicioInverno = new Date(year, 5, 21);
    const inicioPrimavera = new Date(year, 8, 22);
  ...
  // --- FUNÇÕES DE API E CLIMA (com lógica de Risco) ---
  ...
  const getNews = async () => {
    ...
    try {
      ...
      if (data.status !== "ok") { throw new Error("Não foi possível carregar o feed de notícias do G1."); }
      renderNews(data.items);
    } catch (error) {
      console.error("Erro ao buscar notícias via RSS:", error);
      newsCarouselInner.innerHTML = `<div class="carousel-item active"><p class="text-center p-5 text-danger">Erro ao buscar notícias.</p></div>`;
    }
  };
  ...
  // --- FUNÇÕES DE RENDERIZAÇÃO (original) ---
  ...
  // --- FUNÇÃO DE BUSCA PRINCIPAL (MODIFICADA PARA USAR DADOS MOCKADOS) ---
  const search = async () => {
    const city = cityInput.value.trim().toLowerCase(); // Normaliza a entrada para minúsculas
    if (!city) {
      displayError("Por favor, insira o nome de uma cidade de teste.");
      return;
    }
    showLoading(true);
    ...
    const mockCityData = mockApiData[city];

    if (mockCityData) {
        // Se a cidade de teste for encontrada, usa os dados mockados
        cityImage.style.opacity = 0;
        setTimeout(() => {
            cityImage.src = mockCityData.imageUrl;
            cityImage.onload = () => (cityImage.style.opacity = 1);
        }, 300);
        renderResults(mockCityData.weather, mockCityData.pollution);
    } else {
        // Se a cidade não for uma das cidades de teste, exibe um erro
        displayError(`A cidade de teste "${cityInput.value}" não existe. Tente "Cidade Teste 1", "Cidade Teste 2", etc.`);
    }
