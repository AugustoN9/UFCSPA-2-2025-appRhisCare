document.addEventListener("DOMContentLoaded", () => {
 ...
 const getSeasonInfo = () => {
    ...
    // Define as datas de início aproximadas para as estações no Hemisfério Sul
    const inicioVerao = new Date(year, 11, 21); // 21 de Dezembro (mês 11)
    const inicioOutono = new Date(year, 2, 20); // 20 de Março (mês 2)
    const inicioInverno = new Date(year, 5, 21); // 21 de Junho (mês 5)
    const inicioPrimavera = new Date(year, 8, 22); // 22 de Setembro (mês 8)

    let season = "";
    let imagePath = "";
    let message = "";

    // A lógica agora compara as datas exatas
    if (today >= inicioVerao || today < inicioOutono) {
        season = "Verão";
        imagePath = "images/estacoes/estacao02-verao.png";
        message = "Atenção no Verão: Calor e umidade podem influenciar.";
    } else if (today >= inicioPrimavera) {
        season = "Primavera";
        imagePath = "images/estacoes/estacao01-primavera.png"; 
        message = "Estamos na Primavera: Cuidado com a polinização!";
    } else if (today >= inicioInverno) {
        season = "Inverno";
        imagePath = "images/estacoes/estacao04-inverno.png"; 
        message = "Estamos no Inverno: O ar frio e seco exige mais cuidado.";
    } else { // Se não for nenhuma das acima, só pode ser Outono
        season = "Outono";
        imagePath = "images/estacoes/estacao03-outono.png";
        message = "Estamos no Outono: Mudanças climáticas podem impactar.";
    }
  ...
  const renderSeasonDisplay = () => {
    ...
    if (seasonIcon && seasonText && seasonBadge) {
      seasonIcon.src = seasonInfo.imagePath;
      seasonIcon.alt = `Ícone de ${seasonInfo.season}`;
      seasonIcon.style.display = 'block';

      seasonText.textContent = seasonInfo.message;
      seasonBadge.textContent = seasonInfo.season;
      
      seasonBadge.className = 'badge'; // Limpa classes antigas
      if (currentTheme === 'dark') {
          seasonBadge.classList.add('bg-secondary', 'text-light');
      } else {
          seasonBadge.classList.add('bg-primary', 'text-light');
      }
    }
  };

  // --- FUNÇÕES DE SUBMISSÃO DE FORMULÁRIO ---
  // ... (todas as suas funções handle...Submit continuam aqui, inalteradas)
  ...
  const calculateRhisAlert = (weather, air) => {
    let scoreTotal = 0;
    const weatherData = weather.main;
    const windData = weather.wind;
    const airData = air.list[0].components;
    if (!weatherData || !airData || !airData.pm2_5) {
      return { nivel: "Indisponível", score: 0, titulo: "Dados insuficientes para o cálculo.", recomendacoes: [] };
    }
    const temp = weatherData.temp;
    const humidity = weatherData.humidity;
    const wind_speed_kph = windData.speed * 3.6; 
    ...
    if (temp < 12) scoreTotal += 10;
    if (wind_speed_kph > 25) scoreTotal += 5;
    ...
    if (scoreTotal > 45) return { nivel: "Alto", score: scoreTotal, titulo: "Risco Alto: Previna-se!", recomendacoes: ["Reduza atividades físicas intensas ao ar livre.", "Beba bastante água e lave o nariz com soro.", "Tenha sua medicação por perto."] };
    if (scoreTotal > 20) return { nivel: "Moderado", score: scoreTotal, titulo: "Risco Moderado: Fique Atento.", recomendacoes: ["Pessoas sensíveis podem sentir desconforto.", "Mantenha-se hidratado.", "Evite exposição direta a ar condicionado."] };
    return { nivel: "Baixo", score: scoreTotal, titulo: "Risco Baixo: Dia tranquilo!", recomendacoes: ["Condições favoráveis para atividades ao ar livre.", "Continue com seus cuidados de rotina."] };
  };
  
  /**
   * NOVA FUNÇÃO: Define as cores e textos para o medidor
   */
  const getRiskGaugeConfig = (alertData) => {
    const isDark = currentTheme === 'dark';
    const config = {
        'Baixo': { color: '#28a745', textColor: 'text-success' },
        'Moderado': { color: '#ffc107', textColor: 'text-warning' },
        'Alto': { color: '#fd7e14', textColor: 'text-orange' }, // Usando uma cor laranja personalizada
        'Muito Alto': { color: '#dc3545', textColor: 'text-danger' },
        'Indisponível': { color: '#6c757d', textColor: 'text-secondary' }
    };
    const style = config[alertData.nivel] || config['Indisponível'];
    style.emptyColor = isDark ? '#495057' : '#e9ecef';
    return style;
  };

  /**
   * NOVA FUNÇÃO: Desenha o medidor de risco usando Chart.js
   */
  const createRiskGauge = (alertData) => {
    ...
    const gaugeConfig = getRiskGaugeConfig(alertData);
    const score = Math.min(alertData.score, 100); // Garante que o score não passe de 100
    ...
    const chartConfig = {
        ...
        data: {
            datasets: [{
                ...
                backgroundColor: [gaugeConfig.color, gaugeConfig.emptyColor],
                borderColor: [gaugeConfig.color, gaugeConfig.emptyColor],
                borderWidth: 0,
                circumference: 180,
                rotation: 270,
            }]
    ...
    const riskRecommendationsList = document.getElementById('risk-recommendations-list');

    if (riskScoreText) riskScoreText.textContent = `${alertData.score.toFixed(0)} / 100`;
    if (riskLevelText) {
        riskLevelText.textContent = alertData.titulo;
        riskLevelText.className = `h5 mt-2 ${gaugeConfig.textColor}`;
    }
    if (riskRecommendationsList) {
        riskRecommendationsList.innerHTML = alertData.recomendacoes.map(rec => `<li>${rec}</li>`).join('');
    }
  };
  ...
  const getCityImage = async (city) => {
    ...
    if (!unsplashApiKey || unsplashApiKey.includes("SUA_CHAVE")) return defaultImage;
    try {
      ...
      return data.results && data.results.length > 0 ? data.results[0].urls.regular : defaultImage;
  ...
  const renderNews = (articles) => {
    ...
    articles.slice(0, 5).forEach((article, index) => {
      const imageUrl = article.thumbnail || "https://placehold.co/800x400/007782/FFFFFF?text=G1+Notícias";
      newsHtml += `
        <div class="carousel-item ${index === 0 ? "active" : ""}">
          <img src="${imageUrl}" class="d-block w-100 news-image" alt="${article.title}">
          <div class="carousel-caption d-none d-md-block">
            <h5>${article.title}</h5>
            <p><a href="${article.link}" target="_blank" class="btn btn-info btn-sm mt-2">Leia mais no G1</a></p>
          </div>
        </div>
      `;
    });
  ...
  /**
   * FUNÇÃO ATUALIZADA: Renderiza todos os resultados, incluindo o medidor de risco
   */
